...
type ActionHandlers = {
  [K in ActionType]: (event: SingleSocketEvent, data: Extract<Action, { type: K }>) => Promise<void>;
}
...
export class MiniSigameLobby extends SingleReplica {
  public maxUsers: number = 8;
	...
	constructor(
		...
		this.state.blockConcurrencyWhile(() => this.initializeLobby().catch(console.error));
	}

  private readonly handlers: Partial<ActionHandlers> = {
    "lobby:ready": async (event) => {
			const { rid } = event;
			const player = this.lobbyState.players.find(player => player.user.id === rid);
  ...
  async onmessage(state: SingleSocketEvent, data: string): Promise<void> {
	...
	private boradcastHostState() {
		this.whisper(this.manifest.host, JSON.stringify(this.getHostStateMessage()));
	}
}
