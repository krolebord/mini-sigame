...
export async function handlePackUpload({ request, bucket, kv, lobby }: PackUploadOptions) {
			...
			switch (action) {
				case "mpu-uploadpart": {
					...
					try {
						...
						const uploadedPart: R2UploadedPart = await multipartUpload.uploadPart(partNumber, buff);
