...
type UploadPackOptions = Pick< Extract<Awaited<ReturnType<typeof parsePack>>, { success: true }>, 'manifest' | 'packedAssets'> & {
  username: string,
}

const buildUploadUrl = (opts: { action: string, key?: string, uploadId?: string, partNumber?: string }) => {
...
async function uploadFile(key: string, file: Blob) {
  const uploadIdResponse = await fetch(buildUploadUrl({ action: 'mpu-create', key }), {
    method: "POST",
  });
  const uploadId = (await uploadIdResponse.json()).uploadId as string;

  const partCount = Math.ceil(file.size / partSize);
  const uploadedParts = await Promise.all(Array.from({ length: partCount }, async (_, index) => {
    const response = await fetch(buildUploadUrl({ action: 'mpu-uploadpart', key, uploadId, partNumber: (index + 1).toString() }), {
      method: "PUT",
  ...
  const completeResponse = await fetch(buildUploadUrl({ action: 'mpu-complete', key, uploadId }), {
    method: "POST",
